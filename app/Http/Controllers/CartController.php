...
class CartController extends Controller
{
    public function giohang(Request $request)
    {
        ...
        // Kiểm tra nếu số lượng sản phẩm thêm vào vượt quá số lượng có sẵn
        if (($quantity + (isset($cart[$productId]) ? $cart[$productId]['quantity'] : 0)) > $product->product_quantity) {
            return redirect()->back()->with('error', 'Số lượng sản phẩm không đủ, vui lòng chọn số lượng nhỏ hơn hoặc bằng ' . $product->product_quantity);
        }
        ...
        if (isset($cart[$productId])) {
            ...
            $cart[$productId] = [
                ...
                'size' => $request->input('size'), // Lưu size từ request
    ...
    public function xemgiohang()
    {
        ...
        // dd($cart);
        return view('pages.cart', compact('sanpham', 'cart','categories'));
    }
